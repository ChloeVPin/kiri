> **ARCHIVED — superseded by `docs/GAP_MATRIX.md` + `docs/ROADMAP.md` + `docs/COMPETITIVE_ANALYSIS.md`. Retained for history; do not update.**
...
|---|------------------|----------------|----------|
| G-1 | Mobile targets (iOS, Android) via `tauri::mobile_entry_point` | Absent. Desktop only (Linux, macOS, and Windows). | AGENTS.md env facts; `host_windows.rs`/`host_cross.rs` are the only backends. (A) |
...
- [x] Mac-headless-runnable roadmap FULLY CLOSED: every item in §6b (1-18) is done and
      ...
      `cargo test --workspace` = 279 pass (220 kiri-core + 57 kiri-runtime + 2 integration), `bulk_bench` runs. No further Mac-headless-runnable
