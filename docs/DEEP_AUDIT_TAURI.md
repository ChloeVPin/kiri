...
| G-4 | `tauri://` asset protocol with range requests, content-type mapping, optimization, and `asset:customProtocol` allowlist. | We serve `kiri://localhost/index.html` via a per-request `std::fs::read` with NO mime handling, NO caching, NO range support. See `host_cross.rs:45` (`frontend_index_bytes` -> `std::fs::read`). The Windows direct host uses `SetVirtualHostNameToFolderMapping` (folder map, still no custom mime/range). | `crates/kiri-runtime/src/host_cross.rs` lines 38-55. (B) |
