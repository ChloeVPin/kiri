...
| G-5 | `window.__TAURI__.os`, `path`, `app`, `event`, `cli`, `dialog`, `fs`, `globalShortcut`, `http`, `notification`, `process`, `shell`, `updater`, `window` JS APIs. | Only the `kiri.*` control plane + bridge script. No OS/path/app/event JS surface. | `examples/blank/index.html` + `host_cross.rs` bridge. (A) |
...
- [x] R-4 (P1) headless signed-updater DONE: kiri-core::update verifies Ed25519-signed RELEASES.json-style manifests (semver + per-platform asset + signature) on all three OSes with no certs. Thin cargo-bundle packaging step remains (unsigned build CI-verifiable; signed distribution needs Apple Developer + Windows code-sign certs). correctness.yml now runs an updater job (cargo test -p kiri-core update::) on windows/macos/ubuntu; it caught a macOS-biased test fixture (signature only on the darwin asset) that failed on Linux/Windows and was fixed so the verifier is proven on every OS, not just the dev host. correctness.yml now runs an updater job (cargo test -p kiri-core update::) on windows/macos/ubuntu; it caught a macOS-biased test fixture (signature only on the darwin asset) that failed on Linux/Windows and was fixed so the verifier is proven on every OS, not just the dev host.
