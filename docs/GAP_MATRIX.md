...
| G-11 | WebSocket / protocol upgrade | Native `tungstenite` worker transport now supports bounded `ws://` connections to the host-owned local seed allowlist; TLS URLs remain unavailable until a pinned TLS policy is added. | Medium |
