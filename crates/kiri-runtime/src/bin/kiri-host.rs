        ...
        match args[i].as_str() {
            ...
            "--ipc-bench-transport" => {
                ...
                match args.get(i).and_then(|v| kiri_runtime::ipc_bench::IpcBenchTransport::parse(v))
                {
                    ...
                    None => {
                        eprintln!(
                            "unknown --ipc-bench-transport (want default|ring_zerocopy|protocol_ring|protocol_inline)"
                        );
                        std::process::exit(2);
                    }
                }
            ...
            "--help" | "-h" => {
                println!(
                    "kiri-host: native host (cross-platform)\n\
                     ...
                     \x20  [--ipc-bench-transport default|ring_zerocopy|protocol_ring|protocol_inline]\n\
